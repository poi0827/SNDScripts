...
function FisherStateMachine:Reset()
    ...
    self.stateTimeouts = {
        [States.INIT] = 30,           -- 30秒
        [States.CHECK_PLUGINS] = 30,  -- 30秒
        [States.CHECK_JOB] = 30,      -- 30秒
        [States.USE_MEDICINE] = 30,   -- 30秒
        [States.CHECK_INVENTORY] = 30, -- 30秒
        [States.TRAVEL_TO_FISHING] = 180, -- 3分钟
        [States.FISHING] = 7200,      -- 2小时（7200秒）
        [States.STOP_FISHING] = 60,   -- 1分钟
        [States.REPAIR_EXTRACT] = 300, -- 5分钟
        [States.TRAVEL_TO_EXCHANGE] = 300, -- 5分钟
        [States.EXCHANGE] = 600,      -- 10分钟
        [States.ERROR] = 30,          -- 30秒
        [States.COMPLETED] = 10       -- 10秒
    }
    self.attempts = 0
    self.maxAttempts = 3
...
function FisherStateMachine:ShouldTimeout()
    local timeout = self.stateTimeouts[self.state] or 300
    return os.clock() - self.stateStartTime > timeout
end
...
function FisherStateMachine:Execute()
    ...
    if self:ShouldTimeout() then
        local elapsed = os.clock() - self.stateStartTime
        local timeout = self.stateTimeouts[self.state] or 300
        self:HandleError("状态 " .. self.state .. " 超时（运行 " .. string.format("%.1f", elapsed) .. " 秒，超时阈值 " .. timeout .. " 秒）")
        return true
    end
...
function FisherStateMachine:State_FISHING()
    DebugLog("钓鱼状态 - 超时时间: 2小时")
    
    local currentTime = os.clock()
    local elapsedTime = currentTime - self.stateStartTime
    
    -- 每5分钟输出一次状态信息
    if math.floor(elapsedTime) % 300 == 0 then
        DebugLog("钓鱼状态 - 已运行: " .. string.format("%.1f", elapsedTime) .. "秒")
        DebugLog("钓鱼状态 - 剩余时间: " .. string.format("%.1f", 7200 - elapsedTime) .. "秒")
    end
    
    if currentTime - self.lastCheckTime >= self.checkInterval then
       ...
        
        -- 检查鱼饵数量
        if FishingBaitId ~= nil and FishingBaitId ~= 0 then
            local baitCount = GetItemCount(FishingBaitId)
            if baitCount == 0 then
                DebugLog("鱼饵数量为0，停止钓鱼")
                self:TransitionTo(States.STOP_FISHING)
                return true
            elseif baitCount < 100 and math.floor(elapsedTime) % 60 == 0 then
                -- 鱼饵少于100时每分钟提醒一次
                DebugLog("警告: 鱼饵剩余数量: " .. baitCount)
            end
        end
    end
    
...
function FisherStateMachine:State_ERROR()
   ...
    
    -- 如果是钓鱼状态超时，根据运行时间决定恢复策略
    if string.find(self.errorMessage, "状态 FISHING 超时") then
        local elapsed = os.clock() - self.stateStartTime
        if elapsed > 3600 then -- 如果已经钓鱼超过1小时
            DebugLog("钓鱼超过1小时后超时，可能是正常结束，重新检查背包")
            yield("/echo 钓鱼长时间运行后超时，重新检查背包状态")
            self:TransitionTo(States.CHECK_INVENTORY)
            return true
        end
    end
   ...
    
    if currentZoneId == FishingZoneID then
        DebugLog("当前在钓鱼地图，尝试重新开始钓鱼")
        yield("/echo 在钓鱼地图检测到错误，尝试重新开始钓鱼")
        self:TransitionTo(States.TRAVEL_TO_FISHING)
        return true
    end
    
